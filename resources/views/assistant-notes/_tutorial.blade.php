{{-- Modal tutorial Assistant Notes (dipakai Nanny & Majikan).
     Parameter: $steps = [ ['icon'=>.., 'color'=>.., 'title'=>.., 'body'=>html], ... ] --}}
@if(!empty($steps))
<div id="anTutorialModal" class="hidden fixed inset-0 z-50 flex items-end sm:items-center justify-center">
    <div id="anTutorialBackdrop" class="absolute inset-0 bg-[#1E1B2E]/60 backdrop-blur-sm"></div>
    ...
    <div class="relative bg-white rounded-t-[28px] sm:rounded-[28px] w-full max-w-md max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div class="relative bg-[#8B46D3] px-5 pt-5 pb-4">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-white text-[15px] font-extrabold">Panduan Catatan Asisten</span>
                    <p id="anTutorialCount" class="text-white/60 text-[11px] font-bold mt-0.5">Langkah 1 dari {{ count($steps) }}</p>
                </div>
                <button type="button" onclick="anTutorialClose()"
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 hide-scrollbar">
            ...
            <div class="an-step" data-step="{{ $i }}" @if($i !== 0) style="display:none" @endif>
        ...
        <div class="flex items-center justify-center gap-1.5 pt-1">
            @foreach($steps as $i => $s)
            <span class="an-dot w-1.5 h-1.5 rounded-full transition-all" data-step="{{ $i }}"
        ...
        <div class="flex items-center gap-3 p-4 pb-24   ">
            <button type="button" onclick="anTutorialGo(-1)" id="anTutorialPrev"
            ...
            <button type="button" onclick="anTutorialGo(1)" id="anTutorialNext"
...
<script>
(function () {
    if (window.__anTutorialBound) return;
    window.__anTutorialBound = true;

    const modal = document.getElementById('anTutorialModal');
    const total = {{ count($steps) }};
    let cur = 0;

    const stepEls = () => Array.from(modal.querySelectorAll('.an-step'));
    const dotEls  = () => Array.from(modal.querySelectorAll('.an-dot'));
    const countEl = document.getElementById('anTutorialCount');
    const prevBtn = document.getElementById('anTutorialPrev');
    const nextBtn = document.getElementById('anTutorialNext');
    ...
    window.anTutorialOpen = function () {
        cur = 0; render();
        modal.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
    };
    window.anTutorialClose = function () {
        modal.classList.add('hidden');
        document.body.style.overflow = '';
    };
    window.anTutorialGo = function (d) {
        const n = cur + d;
        if (n < 0) return;
        if (n >= total) { anTutorialClose(); return; }
        cur = n; render();
    };

    modal.querySelector('#anTutorialBackdrop').addEventListener('click', anTutorialClose);
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && !modal.classList.contains('hidden')) anTutorialClose();
    });
})();
</script>
@endif
