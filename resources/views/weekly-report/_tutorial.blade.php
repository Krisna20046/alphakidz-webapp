{{-- Modal tutorial Weekly Report (dipakai Nanny & Majikan).
     Parameter: $steps = [ ['icon'=>.., 'color'=>.., 'title'=>.., 'body'=>html], ... ] --}}
@if(!empty($steps))
<div id="wrTutorialModal" class="hidden fixed inset-0 z-50 flex items-end sm:items-center justify-center">
    <div id="wrTutorialBackdrop" class="absolute inset-0 bg-[#1E1B2E]/60 backdrop-blur-sm"></div>
    ...
    <div class="relative bg-white rounded-t-[28px] sm:rounded-[28px] w-full max-w-md max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div class="relative bg-[#8B46D3] px-5 pt-5 pb-4">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-white text-[15px] font-extrabold">Panduan Weekly Report</span>
                    <p id="wrTutorialCount" class="text-white/60 text-[11px] font-bold mt-0.5">Langkah 1 dari {{ count($steps) }}</p>
                </div>
                <button type="button" onclick="wrTutorialClose()"
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 hide-scrollbar">
            ...
            <div class="wr-step" data-step="{{ $i }}" @if($i !== 0) style="display:none" @endif>
        ...
        <div class="flex items-center justify-center gap-1.5 pt-1">
            @foreach($steps as $i => $s)
            <span class="wr-dot w-1.5 h-1.5 rounded-full transition-all" data-step="{{ $i }}"
        ...
        <div class="flex items-center gap-3 p-4 pb-24">
            <button type="button" onclick="wrTutorialGo(-1)" id="wrTutorialPrev"
            ...
            <button type="button" onclick="wrTutorialGo(1)" id="wrTutorialNext"
...
<script>
(function () {
    if (window.__wrTutorialBound) return;
    window.__wrTutorialBound = true;

    const modal = document.getElementById('wrTutorialModal');
    const total = {{ count($steps) }};
    let cur = 0;

    const stepEls = () => Array.from(modal.querySelectorAll('.wr-step'));
    const dotEls  = () => Array.from(modal.querySelectorAll('.wr-dot'));
    const countEl = document.getElementById('wrTutorialCount');
    const prevBtn = document.getElementById('wrTutorialPrev');
    const nextBtn = document.getElementById('wrTutorialNext');
    ...
    window.wrTutorialOpen = function () {
        cur = 0; render();
        modal.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
    };
    window.wrTutorialClose = function () {
        modal.classList.add('hidden');
        document.body.style.overflow = '';
    };
    window.wrTutorialGo = function (d) {
        const n = cur + d;
        if (n < 0) return;
        if (n >= total) { wrTutorialClose(); return; }
        cur = n; render();
    };

    modal.querySelector('#wrTutorialBackdrop').addEventListener('click', wrTutorialClose);
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && !modal.classList.contains('hidden')) wrTutorialClose();
    });
})();
</script>
@endif