{{-- Modal tutorial Learning Progress (dipakai Nanny & Majikan).
     Parameter: $steps = [ ['icon'=>.., 'color'=>.., 'title'=>.., 'body'=>html], ... ] --}}
@if(!empty($steps))
<div id="lpTutorialModal" class="hidden fixed inset-0 z-50 flex items-end sm:items-center justify-center">
    <div id="lpTutorialBackdrop" class="absolute inset-0 bg-[#1E1B2E]/60 backdrop-blur-sm"></div>
    ...
    <div class="relative bg-white rounded-t-[28px] sm:rounded-[28px] w-full max-w-md max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div class="relative bg-[#8B46D3] px-5 pt-5 pb-4">
            <div class="flex items-center justify-between">
                <div>
                    <span class="text-white text-[15px] font-extrabold">Panduan Learning Progress</span>
                    <p id="lpTutorialCount" class="text-white/60 text-[11px] font-bold mt-0.5">Langkah 1 dari {{ count($steps) }}</p>
                </div>
                <button type="button" onclick="lpTutorialClose()"
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 hide-scrollbar">
            ...
            <div class="lp-step" data-step="{{ $i }}" @if($i !== 0) style="display:none" @endif>
        ...
        <div class="flex items-center justify-center gap-1.5 pt-1">
            @foreach($steps as $i => $s)
            <span class="lp-dot w-1.5 h-1.5 rounded-full transition-all" data-step="{{ $i }}"
        ...
        <div class="flex items-center gap-3 p-4 pb-24   ">
            <button type="button" onclick="lpTutorialGo(-1)" id="lpTutorialPrev"
            ...
            <button type="button" onclick="lpTutorialGo(1)" id="lpTutorialNext"
...
<script>
(function () {
    if (window.__lpTutorialBound) return;
    window.__lpTutorialBound = true;

    const modal = document.getElementById('lpTutorialModal');
    const total = {{ count($steps) }};
    let cur = 0;

    const stepEls = () => Array.from(modal.querySelectorAll('.lp-step'));
    const dotEls  = () => Array.from(modal.querySelectorAll('.lp-dot'));
    const countEl = document.getElementById('lpTutorialCount');
    const prevBtn = document.getElementById('lpTutorialPrev');
    const nextBtn = document.getElementById('lpTutorialNext');
    ...
    window.lpTutorialOpen = function () {
        cur = 0; render();
        modal.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
    };
    window.lpTutorialClose = function () {
        modal.classList.add('hidden');
        document.body.style.overflow = '';
    };
    window.lpTutorialGo = function (d) {
        const n = cur + d;
        if (n < 0) return;
        if (n >= total) { lpTutorialClose(); return; }
        cur = n; render();
    };

    modal.querySelector('#lpTutorialBackdrop').addEventListener('click', lpTutorialClose);
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && !modal.classList.contains('hidden')) lpTutorialClose();
    });
})();
</script>
@endif
