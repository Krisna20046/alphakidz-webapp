{{-- resources/views/majikan/diary-choose.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
...
<div class="phone-frame bg-plum-pale flex flex-col">
    ...
    <div class="header-bg relative shrink-0"
        ...
        <div class="flex flex-col items-center anim-up d1">
            ...
            <p style="font-size:14px;color:#F3E6FA;font-weight:500;">Pilih anak untuk melihat diary</p>
        </div>
    </div>

    @if(isset($anakList) && count($anakList) > 0)
    <!-- LIST HEADER -->
    <div class="flex items-center justify-between anim-up d2" style="padding:24px 20px 12px;">
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar" style="padding:8px 20px 0;">
        ...
        <a href="{{ route('majikan-diary', $anak['id']) }}"
           ...
           style="border-radius:16px;margin-bottom:12px;border:2px solid #F3E6FA;padding:16px;
            ...
            <div class="flex-1 min-w-0">
                <p class="line-clamp-1" style="font-size:16px;font-weight:700;color:#4A0E35;margin-bottom:6px;">
                    {{ $anak['nama'] }}
                </p>
                <div class="flex items-center" style="gap:12px;">
                    <div class="flex items-center" style="gap:4px;">
                        <ion-icon name="gift" style="font-size:13px;color:#7B1E5A;flex-shrink:0;"></ion-icon>
                        <span style="font-size:13px;color:#7B1E5A;font-weight:500;">{{ $anak['umur'] ?? '-' }}</span>
                    </div>
                    <div class="flex items-center" style="gap:4px;">
                        @php $m = ($anak['gender']??'') === 'L'; @endphp
                        <ion-icon name="{{ $m ? 'male' : 'female' }}" style="font-size:13px;color:#7B1E5A;flex-shrink:0;"></ion-icon>
                        <span style="font-size:13px;color:#7B1E5A;font-weight:500;">{{ $m ? 'Laki-laki' : 'Perempuan' }}</span>
                    </div>
                </div>
            </div>
            <!-- arrow -->
    ...
    <div class="flex-1 flex flex-col items-center justify-center" style="padding:40px;">
        ...
        <p class="text-center" style="font-size:14px;color:#A2397B;line-height:20px;">Anda belum menambahkan data anak</p>
    </div>
    @endif
