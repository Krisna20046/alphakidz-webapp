...
<div class="sm:w-[390px] sm:min-h-[844px] sm:rounded-[44px] sm:shadow-[0_40px_80px_rgba(124,58,237,0.28),0_0_0_8px_#1a1030,0_0_0_10px_#2d1a50] sm:overflow-hidden bg-[#F0EDFB] min-h-screen flex flex-col relative">
    ...
    <div class="anim delay-1 relative z-10 overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.14),_transparent_34%),linear-gradient(135deg,_#A855F7_0%,_#8B46D3_40%,_#9F58F8_100%)] px-[20px] pt-[54px] pb-[102px]">
        ...
        <div class="relative z-10 flex items-start gap-3">
            ...
            <div class="min-w-0">
                <h1 class="text-white text-[18px] font-extrabold leading-tight">New Assignment</h1>
                <p class="text-white/70 text-[11px] font-semibold mt-0.5 leading-[1.35]">Create Nanny Assignment</p>
            </div>
        </div>
    </div>
...
async function loadNannyProfile() {
    try {
        const res  = await fetch(`${API_BASE_URL}/konsultan-user-detail?id_user=${nannyId}`, { 
...
function renderNannyCard(n) {
    ...
    const foto = n.foto || null;
    const avatar = foto 
        ? `<img src="${foto}" class="w-[62px] h-[62px] rounded-full object-cover border-[3px] border-[#E8DDF9] shadow-[0_4px_10px_rgba(139,70,211,0.18)]">` 
        : fallbackAvatarHtml();
    
    document.getElementById('nannyCard').innerHTML = `
        <div class="flex flex-col items-center text-center pt-1 pb-2">
            <div class="mb-3">${avatar}</div>
            <h2 class="text-[#1E1B2E] font-extrabold text-[15px] leading-tight">${n.name || '-'}</h2>
            <div class="h-px w-full bg-[#E7DCF8] mt-4"></div>
...
function fallbackAvatarHtml() { 
    return `<div class="w-[80px] h-[80px] rounded-full bg-[#F3EEFC] border-[3px] border-[#E8DDF9] shadow-[0_4px_10px_rgba(139,70,211,0.12)] flex items-center justify-center"><ion-icon name="person" style="font-size:28px;color:#8B46D3;"></ion-icon></div>`; 
}

function infoRow(icon, color, bg, label, value) { 
    return `<div class="flex items-start gap-2.5"><div class="w-8 h-8 rounded-[5px] flex items-center justify-center flex-shrink-0" style="background:${bg};"><ion-icon name="${icon}" style="font-size:15px;color:${color};"></ion-icon></div><div class="min-w-0"><p class="text-[#B39BCF] text-[9px] font-black uppercase tracking-[1px]">${label}</p><p class="text-[#1E1B2E] text-[12px] font-extrabold break-words">${value || '-'}</p></div></div>`; 
}

function initDates() {
    const today = new Date();
    const pad = n => String(n).padStart(2, '0');
    document.getElementById('tanggalMulai').value = `${today.getFullYear()}-${pad(today.getMonth() + 1)}-${pad(today.getDate())}`;
...
function refreshDateFieldState() { 
    ['tanggalMulai', 'tanggalSelesai'].forEach(id => { 
        const input = document.getElementById(id); 
        const wrapper = input.closest('.field-card'); 
        if (wrapper) wrapper.classList.toggle('active', !!input.value); 
    }); 
}

function openMajikanModal() { 
    document.getElementById('modalMajikan').classList.remove('hidden'); 
    loadMajikan(); 
}

function closeMajikanModal() { 
    document.getElementById('modalMajikan').classList.add('hidden'); 
    document.getElementById('searchMajikan').value = ''; 
}
...
async function loadMajikan() {
    ...
    try {
        ...
        const json = await res.json(); 
        allMajikan = json.data || []; 
        renderMajikan(allMajikan);
    } catch (e) { 
        list.innerHTML = `<p class="text-center text-red-500 text-sm py-8 font-semibold">Gagal memuat</p>`; 
    }
}

function filterMajikan(q) { 
    const query = q.toLowerCase(); 
    renderMajikan(allMajikan.filter(m => (m.name || '').toLowerCase().includes(query) || (m.email || '').toLowerCase().includes(query))); 
}

function renderMajikan(items) {
    const list = document.getElementById('majikanListModal');
    if (!items.length) { 
        list.innerHTML = `<p class="text-center text-[#8B86A5] text-sm py-8 font-semibold">Tidak ditemukan</p>`; 
        return; 
    }
...
function openAnakModal() { 
    document.getElementById('modalAnak').classList.remove('hidden'); 
    renderAnak(); 
}

function closeAnakModal() { 
    document.getElementById('modalAnak').classList.add('hidden'); 
}

async function loadAnak(idMajikan) {
    try {
        const fd = new FormData(); 
        fd.append('id_majikan', idMajikan);
        const res = await fetch(`${API_BASE_URL}/user-anak-for-konsultan`, { 
            method:'POST', 
            headers:{ 'Accept':'application/json', 'Authorization':`Bearer ${API_TOKEN}` }, 
            body:fd 
        });
...
function renderAnak() {
    ...
    if (!anakList.length) { 
        list.innerHTML = `<p class="text-center text-[#8B86A5] text-sm py-8 font-semibold">Belum ada data anak</p>`; 
        return; 
    }
    list.innerHTML = anakList.map(a => {
        const sel = !!selectedAnak[a.id];
        const today = new Date(); 
        const b = new Date(a.tanggal_lahir); 
...
function toggleAnak(id, nama) {
    if (selectedAnak[id]) delete selectedAnak[id]; 
    else selectedAnak[id] = { id, nama };
    const sel = !!selectedAnak[id]; 
    const row = document.getElementById(`anakRow_${id}`); 
...
function updateAnakDisplay() {
    const count = Object.keys(selectedAnak).length; 
    const names = Object.values(selectedAnak).map(a => a.nama).join(', ');
...
function updateAnakCountLabel() { 
    document.getElementById('anakCountLabel').textContent = `${Object.keys(selectedAnak).length} anak terpilih`; 
}
...
async function handleSubmit() {
    ...
    if (!mulai || !selesai) { 
        showResult(false, 'Data kurang', 'Isi tanggal mulai dan selesai.'); 
        return; 
    }
    if (mulai >= selesai) { 
        showResult(false, 'Tanggal tidak valid', 'Tanggal selesai harus setelah tanggal mulai.'); 
        return; 
    }
    if (!selectedMajikan) { 
        showResult(false, 'Data kurang', 'Pilih majikan terlebih dahulu.'); 
        return; 
    }
    if (!Object.keys(selectedAnak).length) { 
        showResult(false, 'Data kurang', 'Pilih minimal satu anak.'); 
        return; 
    }
    ...
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/nanny-assignment`, { 
            method:'POST', 
            headers:{ 'Accept':'application/json', 'Authorization':`Bearer ${API_TOKEN}` }, 
            body:fd 
        });
        ...
        if (json.status === 'success') {
            showResult(true, 'Berhasil!', 'Penugasan berhasil dibuat.', () => { 
                location.href = '{{ url("/konsultan/tugaskan-nanny") }}'; 
            });
        } else { 
        ...
        showResult(false, 'Kesalahan', 'Terjadi kesalahan jaringan.'); 
        resetBtn(); 
    }
}
...
function resetBtn() { 
    const btn = document.getElementById('btnSubmit'); 
    btn.disabled = false; 
    btn.innerHTML = `<ion-icon name="add-circle-outline" style="font-size:16px;"></ion-icon><span>Create Assignment</span>`; 
}
