...
function renderNannyCard(n) {
    ...
    const avatar = foto
        ...
        : `<div class="w-[80px] h-[80px] rounded-full bg-[#F3EEFC] border-[3px] border-[#E8DDF9] shadow-[0_4px_10px_rgba(139,70,211,0.12)] flex items-center justify-center"><ion-icon name="person" style="font-size:28px;color:#8B46D3;"></ion-icon></div>`;
...
async function handleSubmit() {
    ...
    try {
        ...
        fd.append('id_nanny', nannyId); fd.append('id_majikan', selectedMajikan.id);
        Object.keys(selectedAnak).forEach(id => fd.append('id_anak[]', id));
        fd.append('tanggal_mulai', mulai); fd.append('tanggal_selesai', selesai);
        fd.append('status', currentStatus); fd.append('catatan', catatan || 'Assignment By Konsultan');
...
function resetBtn() {
    const btn = document.getElementById('btnSubmit');
    btn.disabled = false; btn.innerHTML = `<ion-icon name="add-circle-outline" style="font-size:16px;"></ion-icon><span>Create Assignment</span>`;
}
