...
<div class="anim delay-1 relative z-10 bg-[#8B46D3] bg-[url('/assets/bg-texture.png')] bg-cover bg-center
    ...
    <div class="flex items-center gap-3 relative z-10">
        ...
        <div>
            <span class="text-white text-[17px] font-extrabold tracking-wide">Add Contact</span>
...
<div class="flex-1 overflow-y-auto px-[20px] pt-[24px] pb-28 bg-gradient-to-b from-[#F8F7FF] via-[#F8F7FF] to-[#D4BAEF]/50 rounded-t-[50px] -mt-[50px] relative z-20 hide-scrollbar">
    ...
    <form action="{{ route('nanny-emergency-contacts-store') }}" method="POST" class="space-y-4 anim delay-2">
        @csrf
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <input type="text" name="name" value="{{ old('name') }}" class="inp" placeholder="e.g. Ayesha Rahman">
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <input type="text" name="relationship" value="{{ old('relationship') }}" class="inp" placeholder="e.g. Mother / Doctor / Grandparent">
        </div>

        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            <label class="block text-sm font-bold text-[#1E1B2E] mb-2">Phone <span class="text-red-400">*</span></label>
            <input type="tel" name="phone" value="{{ old('phone') }}" class="inp" placeholder="e.g. 08123456701">
        </div>

        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            <label class="block text-sm font-bold text-[#1E1B2E] mb-2">Priority Order <span class="text-[#8B86A5] font-semibold">(1 = most urgent)</span></label>
            <select name="priority_order" class="inp appearance-none">
                @foreach($priorities as $opt)
                <option value="{{ $opt['value'] }}" {{ old('priority_order', '') == $opt['value'] ? 'selected' : '' }}>{{ $opt['label'] }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="flex gap-3 pb-2">
            ...
            <button type="submit" class="act-btn flex-1 py-4 rounded-2xl bg-[#8B46D3] text-white text-sm font-bold shadow-lg shadow-[#8B46D3]/30">Save Contact</button>
        </div>
    </form>
</div>
@endsection