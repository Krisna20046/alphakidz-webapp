...
<div id="historyList" data-idanak="{{ $idAnak }}">
    ...
    <div class="flex flex-col items-center py-8">
        ...
        <p class="text-[#8B86A5] text-xs font-semibold">No notes yet. Add your first note.</p>
    </div>
    @else
    <div class="flex flex-col gap-2">
        ...
        <div class="rounded-2xl border border-[#EAE6F5] p-3 bg-white">
            ...
            <div class="flex justify-end mt-2">
                <button type="button" onclick="anDeleteConfirm('{{ route('nanny-notes-destroy', $r['id']) }}', {{ $r['id'] }})" aria-label="Hapus catatan" class="w-7 h-7 rounded-lg bg-[#FEF2F2] flex items-center justify-center"><ion-icon name="trash-outline" style="font-size:14px;color:#DC2626;"></ion-icon></button>
            </div>
        </div>
        @endforeach
    </div>
