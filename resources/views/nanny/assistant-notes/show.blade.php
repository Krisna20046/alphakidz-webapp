...
{{-- Modal konfirmasi hapus catatan (in-app) --}}
<div id="anDeleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-6">
    <div id="anDeleteBackdrop" class="absolute inset-0 bg-[#1E1B2E]/60 backdrop-blur-sm"></div>
    <div class="relative w-full max-w-sm bg-white rounded-[28px] p-6 text-center shadow-2xl">
        ...
        <p class="text-[16px] font-extrabold text-[#1E1B2E] mb-1">Hapus catatan ini?</p>
        <p class="text-[12px] font-semibold text-[#8B86A5] leading-relaxed mb-5">
            Catatan tidak bisa dikembalikan setelah dihapus.
        </p>
        <form id="anDeleteForm" method="POST">
            @csrf
            @method('DELETE')
            <div class="flex gap-3">
                <button type="button" onclick="anDeleteClose()"
...
<script>
const anDeleteModal = document.getElementById('anDeleteModal');
const anDeleteForm  = document.getElementById('anDeleteForm');
if (anDeleteModal) {
    document.getElementById('anDeleteBackdrop').addEventListener('click', anDeleteClose);
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && !anDeleteModal.classList.contains('hidden')) anDeleteClose();
    });
}
function anDeleteConfirm(url) {
    if (!anDeleteModal) return false;
    anDeleteForm.action = url;
    anDeleteModal.classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    return false;
}
function anDeleteClose() {
    anDeleteModal.classList.add('hidden');
