...
<div class="anim delay-1 relative z-10 bg-[#8B46D3] bg-[url('/assets/bg-texture.png')] bg-cover bg-center
    ...
    <div class="flex items-center gap-3 relative z-10">
        ...
        <div>
            <span class="text-white text-[17px] font-extrabold tracking-wide">Edit Schedule</span>
            <p class="text-white/60 text-xs font-medium mt-0.5">Update school schedule details</p>
        </div>
    </div>
</div>
...
<div class="flex-1 overflow-y-auto px-[20px] pt-[24px] pb-28 bg-gradient-to-b from-[#F8F7FF] via-[#F8F7FF] to-[#D4BAEF]/50 rounded-t-[50px] -mt-[50px] relative z-20 hide-scrollbar">
    ...
    <form action="{{ route('school-schedule.update', $schedule['id']) }}" method="POST" class="space-y-4 anim delay-2" onsubmit="handleSubmit(event)">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            @if(count($children) === 0)
                <p class="text-sm font-semibold text-[#F59E0B]">Tidak ada anak yang ter-assign ke Anda saat ini.</p>
            @else
            <select name="id_anak" class="inp" required>
                <option value="" disabled>Select a child</option>
                @foreach($children as $c)
                <option value="{{ $c['id'] }}" {{ $schedule['id_anak'] == $c['id'] ? 'selected' : '' }}>{{ $c['nama'] }}</option>
                @endforeach
            </select>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            @if(count($subjects) === 0)
                <p class="text-sm font-semibold text-[#F59E0B]">No subjects yet.</p>
            @else
            <select name="subject_id" class="inp" required>
                <option value="" disabled>Select a subject</option>
                @foreach($subjects as $subj)
                <option value="{{ $subj['id'] }}" {{ $schedule['subject_id'] == $subj['id'] ? 'selected' : '' }}>{{ $subj['name'] }}</option>
                @endforeach
            </select>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ substr($schedule['start_time'] ?? '',0,5) }}" class="inp" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-[#8B86A5] mb-2">End <span class="text-red-400">*</span></label>
                    <input type="time" name="end_time" value="{{ substr($schedule['end_time'] ?? '',0,5) }}" class="inp" required>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            <div class="mb-4">
                ...
                <input type="text" name="teacher_name" value="{{ $schedule['teacher_name'] ?? '' }}" placeholder="e.g. Ms. Rina" class="inp">
            </div>
            <div>
                <label class="block text-sm font-bold text-[#1E1B2E] mb-2">Notes</label>
                <textarea name="notes" rows="3" placeholder="Optional" class="inp resize-none">{{ $schedule['notes'] ?? '' }}</textarea>
            </div>
        </div>
...
function handleSubmit(form) {
    ...
    if (!(st && en && en > st)) {
        ...
        d.id = 'errorMsg';
        d.className = 'p-3 rounded-2xl bg-red-50 border border-red-200 text-sm text-red-700 font-bold mb-4';
