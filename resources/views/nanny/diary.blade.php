{{-- resources/views/nanny/diary.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        *{-webkit-tap-highlight-color:transparent;}
        ...
        /* FAB */
        .fab{position:absolute;right:20px;bottom:90px;width:56px;height:56px;border-radius:28px;background:#7B1E5A;display:flex;align-items:center;justify-content:center;box-shadow:0 8px 24px rgba(123,30,90,.4);cursor:pointer;border:none;z-index:40;transition:transform .15s,box-shadow .15s;}
        .fab:hover{transform:scale(1.07);box-shadow:0 12px 28px rgba(123,30,90,.5);}
...
<div class="phone-frame bg-plum-pale flex flex-col" style="max-height:100vh;position:relative;">
    ...
    <div class="header-bg relative flex-shrink-0"
        ...
        <a href="{{ route('nanny-diary-choose') }}"
        ...
        <div class="flex flex-col items-center anim-up d1">
            ...
            <p style="font-size:14px;color:#F3E6FA;font-weight:500;">{{ $diaryData['nama_anak'] ?? '' }}</p>
        </div>
    </div>
    ...
    <div class="flex-shrink-0 anim-up d3" style="padding:0 20px;margin-bottom:16px;">
        <div class="flex items-center justify-between bg-white" style="padding:16px 20px;border-radius:16px;border:2px solid #F3E6FA;">
            ...
            <button id="btnDatePicker" class="flex flex-col items-center flex-1" style="background:transparent;cursor:pointer;border:none;">
                <span style="font-size:16px;font-weight:700;color:#4A0E35;">{{ $tanggalIndo ?? date('j F Y') }}</span>
...
const KATEGORI="{{ $activeKat??$kategori??'' }}";
...
function buildPicker(id,items,active,cb){
    ...
    items.forEach(item=>{
        ...
        d.onclick=()=>{
            ...
            d.classList.add('active');cb(item.v);
...
function buildDayPicker(){
    buildPicker('pickerDay',Array.from({length:daysInMonth(tY,tM)},(_,i)=>({v:i+1,l:String(i+1)})),tD,v=>{tD=v;});
}
