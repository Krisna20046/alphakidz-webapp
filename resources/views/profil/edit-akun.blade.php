...
<style>
    ...
    label.field-label { display: block; font-size: 13px; font-weight: 700; color: #374151; margin-bottom: 8px; }

    .password-wrapper { position: relative; }
    .password-wrapper .inp { padding-right: 45px; }
    .eye-toggle {
        position: absolute; right: 14px; top: 60%;
        transform: translateY(-50%); cursor: pointer; color: #8B46D3;
    ...
    .header-lock {
        ...
        background: #E1CCF4; border-radius: 50%;
    ...
    <form id="editAkunForm" novalidate class="flex flex-col gap-0">
        ...
        <div class="anim delay-2 mb-4">
            ...
            <input type="email" name="email" id="email"
                   ...
                   placeholder="Enter your email" class="inp"/>
        </div>
        ...
        <div class="anim delay-{{ $loop->index + 3 }} mb-4">
            ...
            <div class="password-wrapper">
                <input type="password" name="{{ $f['name'] }}" id="{{ $f['id'] }}"
                       placeholder="{{ $f['placeholder'] }}" class="inp" autocomplete="off"/>
                <button type="button" data-target="{{ $f['id'] }}" class="eye-toggle">
...
<div id="successModal"
    ...
    <div class="modal-box-in w-full sm:max-w-[390px] bg-gradient-to-b from-white via-white to-[#D4BAEF]/40
        ...
        <p class="txt-in-2 text-[#9CA3AF] text-[14px] font-semibold text-center leading-relaxed mb-9 max-w-[240px] z-10">Congratulations, your account has been successfully updated.</p>
...
if (form) {
    form.addEventListener('submit', async (e) => {
        ...
        const csrfToken = form.querySelector('input[name="_token"]')?.value ?? '{{ csrf_token() }}';

        const payload = new FormData();
        payload.append('_token', csrfToken);
        payload.append('email', email);

        // Hanya tambahkan field password jika pengguna memang ingin mengubah password
        if (wantsChangePassword) {
            payload.append('password_lama',             lama);
            payload.append('password_baru',             baru);
            payload.append('password_baru_confirmation', confirm);
        }
