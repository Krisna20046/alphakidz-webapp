...
<div class="flex-1 overflow-y-auto px-[20px] pt-[24px] pb-28 bg-gradient-to-b from-[#F8F7FF] via-[#F8F7FF] to-[#D4BAEF]/50 rounded-t-[50px] -mt-[50px] relative z-20 hide-scrollbar flex flex-col gap-4" id="mainBody">
        ...
        <div class="flex items-center gap-2 mb-5">
            ...
            <div class="flex items-center gap-1.5 opacity-40">
                <div class="w-7 h-7 rounded-full bg-[#EDE9FE] flex items-center justify-center">
                    <span class="text-[#8B46D3] text-xs font-bold">2</span>
                </div>
                <span class="text-[#8B86A5] text-sm font-semibold">Filter</span>
            </div>
        </div>

        <div class="mb-4">
            <div class="flex items-center gap-2 bg-white rounded-full border-2 border-[#EDE9FE] px-4 py-2.5 shadow-sm">
                ...
                />
                <span id="nannyCountBadge" class="hidden bg-[#EDE9FE] text-[#8B46D3] text-xs font-bold px-2 py-0.5 rounded-full badge-pop"></span>
            </div>
        </div>

        <div id="nannyList" class="space-y-3"></div>
    </div>
    ...
    <div id="step2" class="hidden">
        <div class="flex items-center gap-2 mb-5">
            ...
            <div class="flex items-center gap-1.5">
                ...
                <span class="text-[#1E1B2E] text-sm font-bold">Filter</span>
            </div>
        </div>

        <div id="selectedNannyCard" class="bg-white rounded-2xl border-2 border-[#EDE9FE] p-4 mb-5 flex items-center gap-4 shadow-sm">
            <div id="selAvatar" class="w-14 h-14 rounded-2xl bg-[#EDE9FE] flex items-center justify-center overflow-hidden border-2 border-[#EDE9FE] flex-shrink-0">
                <ion-icon name="person" style="font-size:28px;color:#8B46D3;"></ion-icon>
            </div>
            <div class="flex-1 min-w-0">
                <p id="selName" class="text-[#1E1B2E] text-base font-bold truncate">-</p>
                <p id="selEmail" class="text-[#8B86A5] text-xs truncate mt-0.5">-</p>
            </div>
            <button onclick="backToStep1()"
                    class="w-9 h-9 rounded-xl bg-[#EDE9FE] flex items-center justify-center flex-shrink-0 hover:bg-red-50 transition-colors">
                <ion-icon name="close" style="font-size:18px;color:#8B46D3;"></ion-icon>
            </button>
        </div>

        <div class="bg-white rounded-2xl border-2 border-[#EDE9FE] p-5 mb-4 shadow-sm">
            <div class="flex items-center gap-2 mb-5">
            ...
            <div class="mb-4">
                ...
                <div id="startDateWrapper" class="flex items-center gap-2 border-2 border-[#EDE9FE] rounded-xl px-4 py-3 bg-[#F8F7FF] focus-within:border-[#8B46D3] transition-colors">
                    ...
                    <input id="startDate" type="text" readonly
                           ...
                           class="flex-1 text-sm text-[#1E1B2E] bg-transparent outline-none font-bold cursor-pointer"
...
<div id="datePickerModal"
    ...
    <div class="modal-slide w-full sm:max-w-[390px] bg-white rounded-t-3xl shadow-2xl overflow-hidden">
        ...
        <div class="mx-5 mt-4 mb-2 bg-[#EDE9FE] rounded-2xl py-3 px-4 flex items-center justify-center gap-2">
            <ion-icon name="calendar-number-outline" style="font-size:18px;color:#8B46D3;"></ion-icon>
...
const API_BASE_URL = "{{ rtrim(env('API_BASE_URL', ''), '/') }}";
...
function showSkeleton() {
    ...
    list.innerHTML = Array.from({length: 4}).map(() => `
        <div class="bg-white rounded-2xl border-2 border-[#EDE9FE] p-4 flex items-center gap-4">
...
async function fetchNanny() {
    ...
    try {
        ...
        if (data.status === 'success' || data.data) {
            ...
            updateCountBadge(allNanny.length);
        } else {
            showError();
        }
    } catch(e) {
        console.error('Fetch nanny error:', e);
        showError();
    }
}
...
function renderNanny(list) {
    ...
    container.innerHTML = list.map((item, i) => `
        <button onclick="selectNanny('${item.id}')"
                class="card-press w-full text-left bg-white rounded-2xl border-2 border-[#EDE9FE] p-4 flex items-center gap-4 shadow-sm hover:border-[#8B46D3]/40 hover:shadow-md transition-all fade-in"
                style="animation-delay:${i * 0.05}s">
            <div class="w-14 h-14 rounded-2xl overflow-hidden border-2 border-[#EDE9FE] flex-shrink-0 bg-[#EDE9FE] flex items-center justify-center">
                ${item.foto
                    ? `<img src="${item.foto}" class="w-full h-full object-cover" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"/>
                       <ion-icon name="person" style="font-size:26px;color:#8B46D3;display:none;" class="w-full h-full items-center justify-center"></ion-icon>`
                    : `<ion-icon name="person" style="font-size:26px;color:#8B46D3;"></ion-icon>`
                }
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-[#1E1B2E] font-bold text-sm truncate">${item.name}</p>
                <p class="text-[#8B86A5] text-xs truncate mt-0.5">${item.email}</p>
            </div>
            <div class="w-8 h-8 rounded-xl bg-[#EDE9FE] flex items-center justify-center flex-shrink-0">
                <ion-icon name="chevron-forward" style="font-size:16px;color:#8B46D3;"></ion-icon>
            </div>
        </button>
    `).join('');
}
...
function filterNannies(query) {
    ...
    const filtered = q ? allNanny.filter(n =>
...
function selectNanny(id) {
    selectedNanny = allNanny.find(n => n.id === id);
...
function showStep2() {
    ...
    if (selectedNanny.foto) {
        ...
        avatarEl.innerHTML = `<ion-icon name="person" style="font-size:28px;color:#8B46D3;"></ion-icon>`;
    }
...
function buildDateCols() {
    ...
    for (let y = curYear; y >= 2000; y--) {
        ...
        btn.className = `picker-item w-full py-2.5 text-sm font-bold text-center transition-colors ${y === dpYear ? 'bg-[#8B46D3] text-white rounded-xl mx-1' : 'text-[#1E1B2E] hover:bg-[#EDE9FE]'}`;
        btn.textContent = y;
    ...
    MONTHS_ID.forEach((m, i) => {
        const btn = document.createElement('button');
        btn.className = `picker-item w-full py-2.5 text-xs font-bold text-center transition-colors ${i === dpMonth ? 'bg-[#8B46D3] text-white rounded-xl mx-1' : 'text-[#1E1B2E] hover:bg-[#EDE9FE]'}`;
        btn.textContent = m;
        btn.onclick = () => {
            dpMonth = i;
    ...
    for (let d = 1; d <= maxDay; d++) {
        const btn = document.createElement('button');
        btn.className = `picker-item w-full py-2.5 text-sm font-bold text-center transition-colors ${d === dpDay ? 'bg-[#8B46D3] text-white rounded-xl mx-1' : 'text-[#1E1B2E] hover:bg-[#EDE9FE]'}`;
        btn.textContent = d;
        ...
        <button onclick="selectKategori('${opt.value}')"
            ...
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-xl ${selectedKategori === opt.value ? 'bg-white/20' : 'bg-white'} flex items-center justify-center">
                    <ion-icon name="${opt.icon}" style="font-size:16px;color:${selectedKategori === opt.value ? '#fff' : '#8B46D3'};"></ion-icon>
                </div>
...
async function handleGenerate() {
    ...
    try {
        ...
        const res  = await fetch(`${API_BASE_URL}/diary-for-konsultan`, {
            method: 'POST',
