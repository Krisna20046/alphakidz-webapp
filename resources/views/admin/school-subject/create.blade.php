...
<div class="anim delay-1 relative z-10 bg-[#8B46D3] bg-[url('/assets/bg-texture.png')] bg-cover bg-center
    ...
    <div class="flex items-center gap-3 relative z-10">
        ...
        <div>
            <span class="text-white text-[17px] font-extrabold tracking-wide">Add School Subject</span>
            <p class="text-white/60 text-xs font-medium mt-0.5">Create a new school subject</p>
        </div>
    </div>
</div>
...
<div class="flex-1 overflow-y-auto px-[20px] pt-[24px] pb-28 bg-gradient-to-b from-[#F8F7FF] via-[#F8F7FF] to-[#D4BAEF]/50 rounded-t-[50px] -mt-[50px] relative z-20 hide-scrollbar flex flex-col gap-4">
    ...
    <form id="mainForm" action="{{ route('admin-school-subject.store') }}" method="POST"
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <div>
                ...
                <input type="text" name="name" id="fName"
                       value="{{ old('name') }}" placeholder="Example: Mathematics"
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <div class="flex gap-3 overflow-x-auto pb-1 no-scrollbar">
                ...
                @foreach($icons as $iconName => $iconLabel)
                    <button type="button"
                            ...
                            class="icon-opt {{ old('icon') === $iconName ? 'sel' : '' }}"
                            title="{{ $iconLabel }}">
            ...
            <p class="text-[#8B46D3] text-xs mt-2 font-semibold">Saved icon: <span id="iconLabel">-</span></p>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <input type="hidden" name="color" id="colorInput" value="{{ old('color', '#8B46D3') }}">
            ...
            <div class="flex gap-3 flex-wrap">
                @foreach($colors as $c)
                    <button type="button"
                            ...
                            class="color-opt {{ old('color', '#8B46D3') === $c ? 'sel' : '' }}"
            ...
            <div class="flex items-center gap-3 mt-5">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="text" id="colorHexText" value="{{ old('color', '#8B46D3') }}"
        ...
        <div class="bg-white rounded-2xl p-5 border border-[#DDD6EF]">
            ...
            <div class="flex items-center gap-3">
                <div id="previewIcon" class="w-12 h-12 rounded-2xl flex items-center justify-center text-white">
                    <ion-icon name="book-outline" style="font-size:22px;"></ion-icon>
                </div>
                <div>
                    <p id="previewName" class="text-[#1E1B2E] font-bold text-sm">Subject name</p>
                    <p id="previewColor" class="text-[#8B86A5] text-xs mt-0.5">#8B46D3</p>
                </div>
            </div>
        </div>
...
document.getElementById('fName').addEventListener('input', function() {
    document.getElementById('previewName').textContent = this.value.trim() || 'Subject name';
});
...
(function init() {
    ...
    const color = document.getElementById('colorInput').value || '#8B46D3';
    if (icon) document.getElementById('iconLabel').textContent = icon;
